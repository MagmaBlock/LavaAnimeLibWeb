<template>
  <div v-for="collection in collections" class="px-6 sm:px-0 py-1 sm:py-2">
    ...
    <div class="flex mb-4">
      ...
      <div class="flex-wrap">
        <div class="text-xl sm:text-[22px] mr-2 font-medium text-gray-800"> {{ collection.title }} </div>
    ...
    <div class="grid lg:grid-cols-2 gap-4">
      <!-- 单内容块 -->
      <div v-for="content in collection.content" class="bg-gray-50 rounded-md h-fit px-4 lg:px-6 py-3 lg:py-4">
        <!-- 小标题 -->
        <div class="flex flex-nowrap text-gray-600 md:text-base mb-1">
          <div class="w-fit text-zinc-700 font-medium mb-1"> {{ content.title }} </div>
        </div>

...
export default {
  ...
  methods: {
    whatArrayShouldIFor() {
      // if (this.father.selectedVideoList == 'all') 
      return this.father.videoList
      // else return this.father.epVideoList[father.selectedVideoList]
    }
  }
}
</script>
<template>
  ...
  <div v-if="!father.loading">
    ...
    <div class="w-full border rounded-md bg-white opacity-90 px-4 py-2 select-none">
      ...
      <div v-for="video in this.father.epVideoList[father.selectedVideoList] || this.father.videoList" class="ease-in duration-200
        hover:bg-zinc-200 leading rounded-md">

  ...
  >
    ...
    >
      ...
      <div v-if="playType == 'notSupport'">
        当前播放的视频格式可能不受浏览器支持,
        请使用下方的按钮调用您设备上的播放器进行播放
        <span
          class="text-gray-400 hover:text-white cursor-pointer ml-2 text-xs"
          @click="forcePlay()"
          >强制播放</span
        >
      ...
      <div v-if="playType == 'image'" class="grid place-items-center">
        ...
        <div class="text-gray-400 text-xs mt-2">
          这是一张图片附件，可点击上方预览或下载。 大小:
          {{ bytesToSize(video.size) }}
        </div>
        <a
          class="bg-white/20 hover:bg-white/30 text-gray-400 text-[13px] rounded-md w-fit px-4 py-0.5 mt-1"
          target="_blank"
          :href="video.tempUrl"
        >
          下载此文件
      ...
      >
        ...
        您的浏览器不支持该音频格式, 请尝试下载
        <a
          class="bg-white/20 hover:bg-white/30 text-gray-400 text-[13px] rounded-md w-fit px-4 py-0.5 ml-1"
          target="_blank"
          :href="video.tempUrl"
        >
          下载此文件
      ...
      <div v-if="playType == 'notVideo'" class="grid place-items-center">
        ...
        <div class="text-gray-400 text-xs">
          类型: {{ video.extensionName.result }} 大小:
          {{ bytesToSize(video.size) }}
        </div>
        <a
          class="bg-white/20 hover:bg-white/30 text-gray-400 text-[13px] rounded-md w-fit px-4 py-0.5 mt-1"
...
export default {
  ...
  methods: {
      ...
      if (newVideo.type == "file") {
        ...
        if (newVideo.extensionName.type == "video") {
          if (newVideo.extensionName.result == "MP4视频") {
            ...
            saveTime
              ? this.art.switchQuality(newVideo.url)
              : this.art.switchUrl(newVideo.url);
        ...
        // 为暂无法预览的文件类型
          ...
          newVideo.extensionName.type == "subtitle" ||
          newVideo.extensionName.type == "torrent" ||
          newVideo.extensionName.type == "document" ||
          newVideo.extensionName.type == "archive"
        ) {
    ...
    async prepareToReportNewView() {
      // 若某 URL 被持续播放或加载 2 + 5 秒钟，则上报播放量
    ...
    forcePlay() {
      ...
      setTimeout(
        () =>
          (this.art.notice.show = "强制播放可能出现黑屏、无字幕、无声等问题!"),
        300
      );
      setTimeout(
        () =>

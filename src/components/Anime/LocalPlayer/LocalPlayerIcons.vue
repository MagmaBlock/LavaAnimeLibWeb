<template>
  ...
  <LocalPlayerIcon
    ...
    v-if="ua.os.name.match(/Android|Android-x86|HarmonyOS/i) || allos"
  >
  ...
  <LocalPlayerIcon
    ...
    v-if="ua.os.name.match(/Android|Android-x86|HarmonyOS|iOS/i) || allos"
  >
    nPlayer
  </LocalPlayerIcon>
  <!-- MXPlayer -->
...
export default {
  ...
  methods: {
    handleButtonClick(type) {
      ...
      console.log(
        `按钮 ${type} 触发了一次上报播放量，剩余 ${this.reportTimes} 次.`
      );
    },
    getUrl() {
      return {
        ddplayWindows: `ddplay:${encodeURIComponent(
          this.video.url + "|filePath=" + this.video.name
        )}`,

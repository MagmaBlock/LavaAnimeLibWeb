    ...
    <div class="flex flex-wrap gap-1 md:gap-2 p-2">
      ...
      <n-modal v-model:show="moreModel" class="bg-white h-fit">
        <n-card class="max-w-xl" title="全部播放器" :bordered="false" size="small" role="dialog" aria-modal="true">
          ...
          <RouterLink to="/help?article=WhyExternalPlayer" class="text-blue-600 text-xs block mt-2">
            部分视频提示 “需要外部播放器”？
          </RouterLink>
          <RouterLink to="/help?article=ExternalPlayerList" class="text-blue-600 text-xs block">
            可用的外部播放器列表
          </RouterLink>
        </n-card>
      </n-modal>

    ...
    async register() {
      try {
        ...
        let regResult = await LavaAnimeAPI.post('/v2/user/register', {
          email: this.email, password: this.password, name: this.name, inviteCode: this.inviteCode
        })
        if (regResult.data.code == 200) { // 成功注册
          $message.success(regResult.data.message)
          // 开始尝试登录
          let loginResult = await LavaAnimeAPI.post('/v2/user/login', {
            account: this.email, password: this.password
          });
          if (loginResult.data.code == 200) { // 成功登录

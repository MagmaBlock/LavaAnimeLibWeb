  ...
  <div>
    ...
    <n-space class="mb-4">
      <n-avatar class="ring-1 ring-gray-200 dark:ring-zinc-700" fallback-src="/Transparent_Akkarin.jpg" :size="128"
        :src="avatarUrl" />
      <n-avatar class="ring-1 ring-gray-200 dark:ring-zinc-700" fallback-src="/Transparent_Akkarin.jpg" round
    ...
    <n-h3>头像图片链接</n-h3>
    <n-input v-model:value="newAvatarURL" type="url" :input-props="{ autocomplete: 'off' }"
      placeholder="自定义的图片地址, 需要 HTTPS" clearable />
    <n-h3>快速使用第三方头像</n-h3>
    <n-tabs type="line" animated class="m-0">
      <n-tab-pane name="qq" tab="使用 QQ 头像">
        <n-input v-model:value="qqNumber" type="text" placeholder="QQ 号" clearable />
      </n-tab-pane>
      <n-tab-pane name="gravatar" tab="使用 Gravatar 头像">
        <n-space vertical>
          <n-input v-model:value="gravatarEmail" type="email" placeholder="Gravatar 邮箱" clearable />

...
<template>
  <ContainerMobileFull>
    <!-- 开发模式视图 -->
    <div v-if="this.$route.query.dev" class="lg:px-12">
      <AnimeBasicCard class="p-4 mb-4 flex">
        ...
        <RouterLink :to="`/anime/${laID - 1}?dev=1`" class="border border-blue-600 text-blue-600 rounded-md px-2 mx-2">
          前一个
        </RouterLink>
        <RouterLink :to="`/anime/${laID + 1}?dev=1`" class="border border-blue-600 text-blue-600 rounded-md px-2 mx-2">
          后一个
    ...
    <div v-if="!error" class="lg:grid lg:grid-cols-3 lg:gap-6 lg:px-12 w-full">
      <!-- 左视图 占两列 -->
      <div class="lg:col-span-2">
        <!-- 视频框 -->
        <VideoPlayer ref="VideoPlayer" :save-time="saveTime" :video="selectedFile" class="sm:relative sm:mb-4"/>
        <!-- 本地播放器调用 -->
        <LocalPlayers :player="this.$refs.VideoPlayer" :video="selectedFile" class="sm:mb-4"/>
        <!-- 番剧卡，仅在 sm 以上显示 -->
      ...
      <div class="lg:col-span-1">
        <AnimeFileView :la-data="laData" :la-i-d="laID" :selected-file="selectedFile"/>
        <!-- 关联作品 -->
    ...
    <div v-if="error && errorCode == 404" class="w-full grid place-content-center mt-16">
      <n-result class="bg-white/70 w-fit p-10 rounded" description="生活总归带点荒谬" status="404"
                title="404 资源不存在"/>
    </div>
  </ContainerMobileFull>
</template>
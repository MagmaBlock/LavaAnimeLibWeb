<template>
  <div>
    ...
    <DriveSelector
      ...
      :my-drive="myDrive"
      @change-drive="changeDrive"
      v-if="!isDriveListError"
    />
    <FileListLoading v-if="loading && !errorCode"></FileListLoading>
    <FileListMain
      :la-data="laData"
      :file-list="fileList"
      :selected-file="selectedFile"
      v-if="!loading && !errorCode"
      class="sm:mb-4"
    />
  </div>
  <AnimeBasicCard class="py-8 sm:mb-4" v-if="errorCode">
    <n-result
      status="info"
      title="出现错误"
...
export default {
  ...
  methods: {
    ...
    async reborn() {
      ...
      // drive ID list
      let driveIDs = this.driveList.list.map((drive) => {
        return drive.id;
      });
      // 如果客户端自己指定了 drive, 检查一下服务端的 driveList 里还有没有指定的 drive
      if (
        this.myDrive.rememberMyChoice &&
        driveIDs.includes(this.myDrive.selectedDrive)
      ) {
      } else {
        // 如果客户端(被迫)使用默认节点
        this.myDrive.selectedDrive =
          this.driveList.default || this.driveList.list[0].id;
      }
      // 获取相应资源节点下的资源相关
    ...
    async getFileList(laID, drive) {
      try {
        let result = (
          await LavaAnimeAPI.get("/v2/anime/file", {
    ...
    async changeDrive(newDrive) {
      if (this.myDrive.selectedDrive == newDrive) return;
      this.myDrive.selectedDrive = newDrive;
      this.errorCode = null;
      this.isDriveListError = false;
      this.loading = true;

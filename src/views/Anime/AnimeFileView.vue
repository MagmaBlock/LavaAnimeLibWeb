<template>
  <div v-if="!errorCode">
    ...
    <FileListMain v-if="!loading" :file-list="fileList" :la-data="laData" :selected-file="selectedFile"
                  class="sm:mb-4"/>
  </div>
  <div v-else>
    <AnimeBasicCard class="py-8 sm:mb-4">
      <n-result :description="'错误代码: ' + errorCode" status="info" title="出现错误"></n-result>
    </AnimeBasicCard>
  </div>
</template>

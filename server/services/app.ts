...
export class App {
  ...
  constructor() {
    ...
    this.services.registerService(new AnimeInfoService());
    this.services.registerService(new InviteCodeService());
    this.services.registerService(new LibraryService());
    this.services.registerService(new TokenService(authSecret));
    this.services.registerService(
      new UserService(this.services.getService(TokenService))
    );
  }
}
